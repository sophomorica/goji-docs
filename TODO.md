...
- [x] **UI e2e (Cloud env, 2026-08-25):** full loop proven against a LOCAL Supabase stack in the Cursor Cloud VM — kiosk pairing banner → phone claim → wizard (module-scoped math 5 problems + journal 10 words) → Start → School Mode + Today on kiosk (~35s) → drill deck + journal on kiosk → auto-verify → day auto-complete (`all_tasks_done`) + unlock → phone shows Day finished / 2 of 2 + `math.drill` activity. Recipe in `AGENTS.md`; local-stack fixes in `goji-cloud` (seed grants, realtime off locally). **Real-Pi + real-phone smoke still open** (needs paired hardware; same flow)
