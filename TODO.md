...
- [ ] **Parent standing UX** (owner-directed 2026-08-07, spec: [`PARENT_STANDING_UX.md`](./PARENT_STANDING_UX.md)) — glance-first family board (day rings, one attention dot), per-child pillar heat maps over the finite skill catalog (grade-1 phonics: which skills tested/known/weak), evidence sheets with assign-practice, "from their road" wizard mode. Build order in spec §7; step 1 = catalog JSON asset + pure-Dart state computer
...
- [x] 2026-08-07 — **Grade field on children** built: cloud migration `20260807120000_child_grade` (**deploy owed: `npx supabase db push`** — see goji_cloud HUMAN_CHECKLIST), app grade picker + heat-map auto-band (`13703f6`, 70 tests). App degrades gracefully until deploy
