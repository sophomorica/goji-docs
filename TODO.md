...
- [ ] **UI e2e still open (real Pi + phone):** pair on real Pi → wizard/Start on phone → School Mode + Today on kiosk → progress/messages/journal visible on phone
- [x] **Cloud Agent UI loop (2026-08-25):** throwaway anonymous family on live `goji-cloud` → claim this VM’s kiosk → Start → School Mode player (self-confirm + typing) → sync → plan `completed` / `ended_how=all_tasks_done` → parent web joins via invite and shows **Day finished**. Playbook: `CLOUD_AGENT_SCHOOL_DAY.md`. Leftover product gaps: family-board card stays “Not started” after a completed day; board does not refresh after Join until reload.
