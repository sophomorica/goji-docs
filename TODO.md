...
- [ ] **Invite/recovery code — NOW URGENT** (sync-audit item, bit twice 2026-08-07): every app reinstall wipes the local session → new anonymous user → forked empty family. Interim guard: `goji_cloud/scripts/adopt_orphan_sessions.py` (auto-adopts provably-empty orphan families into the real one; running as a session monitor tonight). Real fix: family invite/recovery code at bootstrap
