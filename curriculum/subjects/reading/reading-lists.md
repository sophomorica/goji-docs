...
|-------|--------|----------------------|------|------------|
| The Tale of Peter Rabbit | Beatrix Potter | 14838 | Read-aloud → first independent read; retell practice (`obj.reading.k2.comp.01`) | ✓ `peter-rabbit` |
...
| Mother Goose (illustrated collections) | trad. | 10607 | Rhyme/phonemic awareness read-aloud | ✓ `mother-goose` |
...
| The Story of the Three Little Pigs | trad./Jacobs | 18155 | Beginning/middle/end retell | ✓ `three-little-pigs` |
| English Fairy Tales | Joseph Jacobs | 7439 | Read-aloud anthology; prediction practice | ✓ `english-fairy-tales` |
...
| Heidi | Johanna Spyri | 1448 | Setting/contrast comprehension | ✓ `heidi` |
| The Adventures of Pinocchio | Carlo Collodi | 500 | Cause-and-effect chains | ✓ `pinocchio` |
...
| Robinson Crusoe (abridged path OK) | Daniel Defoe | 521 | Problem→solution structure; older prose on-ramp | ✓ `robinson-crusoe` |
| Little Women | Louisa May Alcott | 514 | Long-novel stamina; four-character contrast | ✓ `little-women` |
| The Adventures of Huckleberry Finn | Mark Twain | 76 | Voice/dialect; moral growth (parent-guided discussion) | ✓ `huckleberry-finn` |
| The Swiss Family Robinson | J. D. Wyss | 11703 | Survival problem-solving; family read | ✓ `swiss-family-robinson` |
| The Adventures of Tom Sawyer | Mark Twain | 74 | Irony and point of view | ✓ `tom-sawyer` |
| Around the World in Eighty Days | Jules Verne | 103 | Geography crossover (Kiwix: countries en route) | ✓ `around-world-80-days` |
| A Christmas Carol | Charles Dickens | 46 | Short Dickens — flashback structure; theme | ✓ `christmas-carol` |
...
| Title | Author | Gutenberg # (verify) | Role | On device? |
|-------|--------|----------------------|------|------------|
| Pride and Prejudice | Jane Austen | 1342 | Social subtext, irony; essay prompts (writing pillar) | ✓ `pride-prejudice` |
| Jane Eyre | Charlotte Brontë | 1260 | First-person voice; moral dilemma essays | ✓ `jane-eyre` |
...
| Frankenstein | Mary Shelley | 84 | Frame narrative; ethics essays; science crossover | ✓ `frankenstein` |
| The Picture of Dorian Gray | Oscar Wilde | 174 | Theme/aesthetics argument writing | |
| Poe — complete tales (selections) | Edgar Allan Poe | 2147 | Short-form close reading; mood/tone analysis | ✓ `poe-tales` |
| The Federalist Papers / founding docs (selections) | various | 1404 | Non-fiction argument reading (civics crossover) | ✓ `federalist-papers` |
...
**29 books live in the reader's library** (ingest run 2026-08-07, goji_computer `dbd2a07` — 16 added incl. the full 9–12 shelf; was 13) (`goji_computer` SQLite `books` table) — all marked ✓ above. Facts from the engineering audit:
...
- ~~In the manifest but not imported~~: `huckleberry-finn` + `swiss-family-robinson` ingested 2026-08-07 (the latter's manifest pointed at the wrong Gutenberg number — fixed).
...
- [x] Ingest priorities *(done 2026-08-07 — all tiers; remaining wishlist: Benjamin Bunny, Five Children and It, Great Expectations, Scarlet Letter, Dorian Gray)*
